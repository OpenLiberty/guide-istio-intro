...
:page-seo-description: A getting started tutorial on how to manage Java microservice traffic with an Istio service mesh and using Kubernetes blue-green deployment as an example.
...
The command builds a {docker} image for the `system` microservice. The `-t` flag in the `docker build` command allows the Docker image to be labeled (tagged) in the `name[:tag]` format. 
...
The [hotspot=testAppVersion]`testAppVersionMatchesPom` test case verifies that the correct version number is returned in the response headers.
