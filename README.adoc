...
CLI tool to deploy your application as well as configure the routing. You will use the `istioctl` tool
...
{istio} and use the given command to produce an `istio.yaml` file that will be setup to work in the `istio-system` namespace, and have sidecar injection disabled. Enabling sidecar injection allows you to deploy your resources without using the `istio-inject` command later
...
{kube} resources defined in a yaml file. This will create some services and deployments that are needed by {istio}.
...
Navigate to the `start` directory and run the given command, it may take a few minutes to build.
...
Use this curl command a few times to make multiple requests `curl http://<ip-address>:<port>/inventory/systems`
