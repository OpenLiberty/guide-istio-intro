...
The microservice you'll deploy is called `hello`. It responds with a JSON object that has a version number and a greeting.

You will use the `kubectl` CLI tool to deploy and manage your {kube} resources. You'll use the `istioctl` CLI tool to inject {istio} sidecars into your pods.

=== What is {istio}?

{istio} is a platform for managing how microservices interact with each other and the outside world.
It works by deploying istio control plane. Then, it injects an extra container into each of your pods that contains
the https://www.envoyproxy.io/[Envoy] proxy. You can think of Envoy as a sidecar that intercepts and controls all the traffic to and from your container. It's specialized for a service mesh architecture since it offers many useful features.
Features it offers such as load balancing and support for distributed tracing help creating a service mesh.

While {istio} supports {kube} and that will be the focus of this guide. Note that {istio}
can also be used with other environments such as Docker Compose. {istio} has many features. Some of these features are
...
Once you see that the microservice is working on the testing site, modify the `traffic.yaml` file to shift 100 percent of the live traffic to the green deployment.
...
The `dockerfile.skip=true` flag skips rebuilding the docker images. The `cluster.ip` and `port`
