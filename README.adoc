...
Next, deploy {istio} to your cluster. From the directory where you installed {istio}, run the following command:
```
...
Ensure that all istio pods are all ready before you continue.
[source, role="no_copy"]
----
...
Therefore, any clients consuming the updated service will now be broken since they are expecting a property called
`total` instead of `count`. Instead of running only the new service, both services can be run simulatenously.
...
Make a request to the service and observe that requests go to both v1 and v2. This is not what
...
to version 1 and allow the client to access version 2 when specifically requested.
...
Make a request `curl http://ip-address:port/api/name -H "x-version: v2"`, observe that request goes to v2 of the service where you see "count".
Make a request `curl http://ip-address:port/api/name`, observe that request default to v1 of the service where you see "total".
