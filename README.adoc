...
It responds with current system's JVM properties and it returns the app version in the response header.
You will increment the version as part of updating the application.
This number will allow you to view which version of the microservice is running in your production or test environments.
...
When you want to deploy a new version of your application, you will deploy to the color that is acting as your test environment.
After the new version is verified on the test environment, the traffic will be shifted over.
...
`src/main/java/io/openliberty/guides/system/SystemApplication.java`
----

Filename
[source, Java, linenums, role='code_column hide_tags=copyright']
----
include::finish/src/main/java/io/openliberty/guides/system/SystemApplication.java[]
----

The `system` microservice is set up to respond with the version that is set in the `SystemResource.java` file.
The tag for the {docker} image is also dependent on the version specified in the `SystemResource.java` file.
Manually update the version of the microservice to `2.0-SNAPSHOT`.
...
One of the environments is a test site located at `test.example.com`.
The other environment is your production environment located at `example.com`.
To start with, the production environment is tied to the blue deployment and the test environment is tied to the green deployment.
