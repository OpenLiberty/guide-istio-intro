...
Before you begin, make sure to have these tools installed:
...
Minikube is a tool to run a kubernetes cluster locally on your computer, inside a virtual machine.
...
First, deploy the {istio} custom resource defintion. Custom resource definitions allows {istio} to define custom {kube} resources that you can use in your resource defintion files.

```
kubectl apply -f install/kubernetes/helm/istio/templates/crds.yaml
```

Next, prepare your {istio} deployment using Helm. Helm is used only for templating, in this situation Helm is not used for
managing the resources. Navigate to the directory where you downloaded and extracted
{istio} and use the given command to produce an `istio.yaml` file that will be setup to work in the `istio-system` namespace.
```
...
Next, deploy {istio}'s resource to your cluster by running the `kubectl apply` command, which creates or updates
...
To deploy the `inventory` and `system` services to the {kube} cluster, run the commands:

```
kubectl apply -f inventory-app.yaml
```
...
Next deploy the virtual services for the `inventory` and `system` service. A VirtualService defines how requests are routed to your applications through the gateway.

```
kubectl apply -f routing.yaml
```

The `routing.yaml` configuration file contains configuration for two VirtualService resources, they specify that requests to `/inventory` be forwarded to the `inventory` service and requests to `/system` are forwarded to the `system` service. Similarly to `inventory-app.yaml` you can see the results of the `kubectl apply -f ...` command for `routing.yaml`:
...
Next, find out what port the `istio-ingressgateway` service is running on. This gateway is the entry point for any HTTP requests that you make to your services.
```
...
istio-ingressgateway   LoadBalancer   10.100.214.161   <pending>     80:31380/TCP,443:31390/TCP,31400:31400/TCP,15011:32250/TCP,8060:31731/TCP,853:31999/TCP,15030:30030/TCP,15031:32585/TCP   2h
----

So the port in this specific scenario would be `31380` since it maps to the internal port `80`. To get the base url of the service
...
```
curl -HHost:my-inventory.com http://<ip-address>:<port>/inventory/systems
```
...
Use this curl command a few times to make multiple requests `curl -HHost:my-inventory.com http://<ip-address>:<port>/inventory/systems`
...
Create the `routing.yaml` file in the `start` directory:
...
```
curl http://<ip-address>:<port>/inventory/systems -HHost:my-inventory.com -Hx-version:v2
```

Make a request and observe that the request defaults to v1 of the service where you see "total".
```
curl http://<ip-address>:<port>/inventory/systems -HHost:my-inventory.com
```
...
== Tearing down your deployments and {istio}
