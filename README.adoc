...
[.tab_link][.windows_link]`*WINDOWS*`
[.tab_link][.mac_link]`*MAC*`
[.tab_link][.linux_link]`*LINUX*`
...
Navigate to the `start` directory and run the following command. It might take a few minutes to complete.
It builds the application and then packages it into a {docker} image.
...
The [hotspot=55-63]`testAppVersionMatchesPom` test case verifies that the correct version number is returned in the response headers.
...
Delete your resources from the cluster. This will stop any running pods created by the yaml files and delete them from the cluster.
