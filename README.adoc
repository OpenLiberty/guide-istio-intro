...
The microservice you'll deploy is called `system`. It responds with current system's JVM properties and it returns app version in the response header. The version will be automatically incremented when you update the `pom.xml` version. This number will allow you to view which version of the microservice is running in your production or test environments.
...
[role=command]
```
curl -HHost:example.com -I http://localhost/system/properties
```
...
[role=command]
```
curl -HHost:example.com -I http://`minikube ip`:31380/system/properties
```
****
...
[role=command]
```
curl -HHost:test.example.com -I http://localhost/system/properties
```
...
[role=command]
```
curl -HHost:test.example.com -I http://`minikube ip`:31380/system/properties
```
****
